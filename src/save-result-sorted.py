...
def parse_args():
	...
	parser.add_argument('--diseaseSNP', nargs='?', default='example_data/diseaseSNP', help='The relationship between disease and SNP, from left to right is [proxySNP, indeSNP, disease] three columns.')
	parser.add_argument('--network', nargs='?', default='example_data/network/', help='Original network data directory')
	parser.add_argument('--label', nargs='?', default='example_data/iteration-1/network.pred_label', help='Predict node label file')
	parser.add_argument('--output', nargs='?', default='example_data/iteration-1/result', help='Result file')
...
for label in labellist:
	...
	module = module[['node_name_x', 'node_name_y']] #module

	#each row
	temp = [module_nodes.shape[0], module.shape[0], module.shape[0]/(module_nodes.shape[0]*(module_nodes.shape[0]-1)/2)]
	for disease in diseaselist:
