...
result = pd.DataFrame(columns=['module_nodes_count', 'module_edges_count'])
...
for label in labelList:
	...
	#each row
	temp = [module_nodeset.shape[0], module.shape[0]] #module的节点集大小，边集大小
	for disease in diseaseList:
...
##save result
#result.sort_values(['SNP_count'], ascending=[0], inplace=True) #sort
